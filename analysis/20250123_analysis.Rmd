...
```{r}
library(dplyr)
meta_updated <- read.table("~/cell_type/20250418_eckerhuman.tsv",sep="\t")
...
subset_cells_label <- subset_cells_label$subtype
PlotConfusion(prediction_result, subset_cells_label, log2 = F)
#ggsave("~/figures/fuh1/2025423_Zhou2025_confusion.pdf",width=10,height=9.5)

predicted_labels <- unique(prediction_result$prediction_label)
n_labels <- length(predicted_labels)
label_groups <- split(predicted_labels, cut(seq_along(predicted_labels), 6, labels = FALSE))

for (i in 1:6) {
  current_preds <- label_groups[[i]]
  # Subset rows where prediction is in current group
  idx <- which(prediction_result$prediction_label %in% current_preds)
  # Subset predictions and actuals
  pred_sub <- prediction_result[idx,]
  label_sub <- subset_cells_label[idx]
  PlotConfusion(pred_sub, label_sub, log2 = F)
  #ggsave(paste0("~/figures/fuh1/2025423_Zhou2025_confusion",i,".pdf"),width=7,height=6.5)
}
...
umap_plot <- PlotUMAP(input_pattern,prediction_result)
umap_plot[[1]]
ggsave("~/figures/fuh1/20250423_Zho2025_umap.pdf",width=6.5,height=4)
...
ggplot(df, aes(x = prediction_status, y = confidence_score, fill = prediction_status)) +
  ...
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_blank())
ggsave("~/figures/fuh1/20250423_Zhou2025_CI.pdf",width=3,height=3)
```
