...
COLUMN_INDEX=31 # column index for the cell type label
SAMPLE_INDEX=2 # column index for the sample id that match with .cg file
...
### use parallel
cut -f"$COLUMN_INDEX" "$path" | sort -u | \
